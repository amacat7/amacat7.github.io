...
Emails.prototype.type = "emails";

/**
 * Our TrumpSigns
 */
function TrumpSigns(pos) {
    this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
    this.size = new Vector(1.5, 1);
    this.wobble = Math.random() * Math.PI * 2;
}
TrumpSigns.prototype.type = "trumpSigns";
...
function DOMDisplay(parent, level) {
    var element = elt("div", "game");
    element.setAttribute("id", "game");
    this.wrap = parent.appendChild(element);
    this.level = level;
...
DOMDisplay.prototype.scrollTrumpIntoView = function () {
    ...
    if (center.y > height / 2) {
        this.wrap.scrollTop = center.y - 200;
    }
    else if (center.y < height / 2) {
        this.wrap.scrollBottom = center.y;
    }
};
...
/**
 * Emails moves in a wobbly sort of way
 *
 * @param step
 */
Emails.prototype.act = function (step) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
/**
 * Trump Signs moves in a wobbly sort of way
 *
 * @param step
 */
TrumpSigns.prototype.act = function (step) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.trumpTouched = function (type, actor) {
    if (type == "lava" && this.status == null) {
        ...
        overworld.pause();
        death.play();
    } else if (type == "money" || type == "emails" || type == "trumpSigns") {
        console.log("Collect!");
        ...
        if (!this.actors.some(function (actor) {
                return actor.type == "money" || actor.type == "trumpSigns" || actor.type == "emails";
            })) {
            this.status = "won";
            this.finishDelay = 1;
        }
 ...
 * @param andThen
 */
function runLevel(level, levelInt, Display, andThen) {
    var levelScreen = document.getElementById("levelScreen");
    console.log(levelInt);
    if (levelInt == 0) {
        levelScreen.style.background = "url('img/level_one.png') no-repeat center";
        levelScreen.style.backgroundSize = "100% 100%";
    } else if (levelInt == 1) {
        levelScreen.style.background = "url('img/level_two.png') no-repeat center";
        levelScreen.style.backgroundSize = "100% 100%";
    } else if (levelInt == 2) {
        levelScreen.style.background = "url('img/level_three.png') no-repeat center";
        levelScreen.style.backgroundSize = "100% 100%";
    }
...
function runGame(plans, Display) {
    ...
    function startLevel(n) {
        runLevel(new Level(plans[n]), n, Display, function (status) {
            ...
            if (status == "lost") {
                ...
                function startOver(event) {
                    if (event.keyCode == 13) {
                        ...
                        while (winScreen.firstChild) {
                            winScreen.removeChild(winScreen.firstChild);
                        }
                        startLevel(0);
                        overworld.currentTime = 0;
                        overworld.play();
            ...
            //<div id="winScreen" class="winScreen"><h1>Congrats! You made it! Play for the Win</h1><iframe id="winframe" width="560" height="315" src="https://www.youtube.com/embed/6zhVZEGKzfk" frameborder="0" allowfullscreen></iframe></div>

                var winScreen = document.getElementById("winScreen");
                winScreen.style.display = "block";
                var iframe = document.createElement('iframe');
                iframe.src = "https://www.youtube.com/embed/6zhVZEGKzfk?autoplay=1";
                iframe.width = "640px";
                iframe.height = "480px";
                winScreen.appendChild(iframe);
